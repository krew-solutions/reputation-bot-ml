...
let handle (type uow) (deps : uow Deps.t) (uow : uow) (cmd : t) =
  ...
  (* Check if chat already mapped *)
  let* existing =
    IdMapping.find_chat_id uow cmd.external_chat_id
  ...
  | None ->
      ...
      let* chat_id =
        ...
        |> map_error (fun e -> Domain_error.Invalid_argument e)
      in
      let chat = Chat.create ~id:chat_id ~community_id:cmd.community_id in
      let* () = ChatRepo.save uow chat ~expected_version:0 in
      (* Save external ID mapping *)
      let* () =
        IdMapping.save_chat_mapping uow cmd.external_chat_id chat_id
        |> map_error (fun e -> Domain_error.Invalid_argument e)
      in
      (* Attach to community *)
